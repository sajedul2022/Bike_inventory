...
<body>
        ...
        <thead>
            ...
            <tr class="text-center">
                <th width="30%" colspan="1">
                    <span>Invoice Id: {{ $purchases[0]->purchase_invoice_no }} </span>
                </th>
            ...
            <tr class="bg-blue">
                <th width="100%" colspan="4">Seller Details</th>
            </tr>
        </thead>
    </table>
    ...
    <table class="table">
        ...
        <tbody>
            <tr>
                {{-- <th scope="row">1</th> --}}
                <td style="width: 90px">{{ $suppliers[0]->supplier_name }}</td>
                <td style="width: 70px">{{ $suppliers[0]->father_name }}</td>
                {{-- <td>{{$suppliers[0]->email}}</td> --}}
                <td>{{ $suppliers[0]->phone }}</td>
                <td>{{ $suppliers[0]->nid }}</td>
                <td style="width: 60px">{{ $suppliers[0]->dob }}</td>
                <td style="width: 90px">{{ $suppliers[0]->address }}</td>
                <td style="width: 80px">{{ $suppliers[0]->mediator_name }}</td>
                <td style="width: 90px">{{ $suppliers[0]->mediator_phone }}</td>
            </tr>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td width="10%"> {{ $purchases[0]->product_id }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_quantity }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_rate }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_amount }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_vat }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_discount }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_amount_paid }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_balance_due }} </td>
                <td width="10%"> {{ $purchases[0]->purchase_total_amount }} </td>

            </tr>

            <tr class="allBorder">
                <td colspan="4" class="grand total">GRAND TOTAL</td>
                <td class="grand total"> = {{ $purchases[0]->purchase_total_amount }} TK</td>
            </tr>

            <tr class="allBorder" >
                <td colspan="4">Paid</td>
                <td class="total"> = {{ $purchases[0]->purchase_amount_paid }} TK</td>
            </tr>

            <tr class="allBorder">
                <td colspan="4">(-) Due</td>
                <td class="total"> = {{ isset($purchases[0]->purchase_balance_due) ? $purchases[0]->purchase_balance_due : 0 }} TK </td>
            </tr>
    ...
    <table>
        <tr class="">
            ...
            <th width="30%" colspan="2">  </br> Buyer's signature </th>
            <th width="30%" colspan="2"> </br> Seller's signature</th>
        </tr>
    </table>
